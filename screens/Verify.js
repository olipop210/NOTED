import { StyleSheet, Text, View, Dimensions, SafeAreaView, Alert, TextInput, Keyboard } from 'react-native';
...
export default function Weryfikacja({ navigation }) {
    ...
    function SprawdzZalogowanie() {
        try {
            AsyncStorage.getItem('verifyEmail')
                .then(value => {
                    if (value != null) {
                        setEmail(value);
                    }
                    else {
                        Alert.alert('Błąd', 'Weryfikacja przerwana');
                    }
                });
    ...
    async function wysylanie(){
        ...
        if(kod > 100000){
            var link = LINK + "notedMobile/Register/Verify.php"; 
            ...
            .then((response)=>{ //jesli wszystko git, wyswietla 0 obiekt z tablicy - czyli odpowiedz
                ...
                let tmp = response.indexOf('Message') + 9;
                let qt1 = response.indexOf('"', tmp);
                let qt2 = response.indexOf('"', qt1+1);
                let wiadomosc = response.slice(qt1 + 1, qt2);
                //responseProf = responseProf.replace('\\', '');

                alert(wiadomosc);
                try {
                AsyncStorage.setItem('loggedEmail', email); //przechowaj w pamięci urządzenia dane o zalogowanym uzytkowniku
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Text style={styles.tekst1}>Weryfikacja adresu E-mail</Text>
